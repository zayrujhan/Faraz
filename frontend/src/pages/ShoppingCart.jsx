...
export default function ShoppingCart() {
  ...
  return <div className="min-h-screen bg-gray-50"><Navbar /><main className="max-w-5xl mx-auto bg-white border border-gray-200 p-6 md:p-10 shadow-sm mt-6">
    ...
    {status ? <div className="text-gray-600">{status} {!getToken() && <Link className="underline font-medium" to="/login">Login</Link>}</div> : <div className="grid grid-cols-1 md:grid-cols-3 gap-12"><section className="md:col-span-2"><h1 className="text-base font-bold text-gray-900 mb-6">Shopping Cart</h1>
      {cart.items.length === 0 ? <p className="text-sm text-gray-500">Your cart is empty. <Link to="/shop" className="underline">Browse products</Link></p> : cart.items.map((item) => <article key={item.id} className="flex items-start justify-between pb-6 mb-6 border-b border-gray-100"><div><Link to={`/products/${item.product.id}`} className="text-sm font-bold text-gray-800">{item.product.name}</Link><p className="text-xs text-gray-500 mt-1 max-w-sm">{item.product.description}</p><p className="text-sm font-bold text-gray-800 mt-3">{formatPrice(item.product.price)}</p><button onClick={() => removeItem(item.id)} className="mt-2 text-xs text-red-600 underline">Remove</button></div><select value={item.quantity} onChange={(e) => changeQuantity(item, Number(e.target.value))} className="border border-gray-300 rounded px-2 py-1 text-xs bg-white">{Array.from({ length: Math.max(1, Math.min(item.product.stock, 10)) }, (_, i) => <option key={i + 1} value={i + 1}>{i + 1} pcs</option>)}</select></article>)}</section>
