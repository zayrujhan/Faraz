...
export default function OrderList() {
  ...
  const loadOrders = useCallback(async () => {
    setLoading(true)
    try {
      const params = { status: status || undefined, search: search || undefined, skip: (page - 1) * PAGE_SIZE, limit: PAGE_SIZE }
  ...
  const columns = [
    ...
    { key: 'total', label: 'Total', align: 'right', render: (row) => <span className="font-medium">{formatPrice(row.total_amount)}</span> },
    { key: 'status', label: 'Status', render: (row) => <StatusBadge status={row.status} /> },
    { key: 'created_at', label: 'Date', render: (row) => <span className="text-gray-500">{formatDate(row.created_at)}</span> },
  ]
      ...
      {loading ? (
        ...
        <>
          <DataGrid
            columns={columns}
            data={orders}
            onRowClick={(row) => window.location.hash = `#/seller/orders/${row.id}`}
