...
export default function ProductList() {
  ...
  const loadProducts = useCallback(async () => {
    setLoading(true)
    try {
      const params = { search, category_id: categoryFilter || undefined, sort, order: 'desc', skip: (page - 1) * PAGE_SIZE, limit: PAGE_SIZE, include_inactive: true }
  ...
  const columns = [
    ...
    { key: 'price', label: 'Price', align: 'right', render: (row) => formatPrice(row.price) },
    { key: 'stock', label: 'Stock', align: 'right', render: (row) => (
    ...
    { key: 'actions', label: '', align: 'right', width: 'w-28', render: (row) => (
      <div className="flex items-center justify-end gap-1">
        <Link to={`/seller/products/${row.id}`} className="p-1.5 rounded hover:bg-gray-100" title="Edit">
          <Edit className="w-4 h-4 text-gray-500" />
        </Link>
        <button onClick={(e) => { e.stopPropagation(); toggleActive(row) }} className="p-1.5 rounded hover:bg-gray-100" title={row.is_active ? 'Deactivate' : 'Activate'}>
          {row.is_active ? <EyeOff className="w-4 h-4 text-gray-500" /> : <Eye className="w-4 h-4 text-gray-500" />}
        </button>
        <button onClick={(e) => { e.stopPropagation(); deleteProduct(row) }} className="p-1.5 rounded hover:bg-gray-100" title="Delete">
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="space-y-2">{[1,2,3,4,5].map(i => <div key={i} className="h-14 bg-gray-100 rounded animate-pulse" />)}</div>
      ) : (
        <>
          <DataGrid
            columns={columns}
            data={products}
            onRowClick={(row) => window.location.hash = `#/seller/products/${row.id}`}
